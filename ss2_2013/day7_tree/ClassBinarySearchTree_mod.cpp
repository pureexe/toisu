#include<iostream>
#define R_MODE 0 /// 0 = while && 1 == recursive
#define R_MUL 0  /// 0 = Only standard && 1 == MultiSet Enabled
using namespace std;
/// Tree = Binary Tree :P
...
public:
    ...
    TreeNode* remove(Object key,TreeNode* head) {
        ...
        if(delnode==NULL)
            return NULL;
        if(delnode==root&&delnode->right==NULL&&delnode->left==NULL){
            delete delnode;
            return NULL;
        }
        if(delnode->key==195)
            cout << "SSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSS";
        if(delnode->count>1) {
            delnode->count--;
            return NULL;
